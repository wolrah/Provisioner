...
class endpoint_polycom_vvx_phone extends endpoint_polycom_base {
    ...
    public $family_line = 'vvx';
    private $configfiles = array();
    public $directory_structure = array("logs", "overrides", "contacts", "licenses", "VVXLocalization", "languages");
    ...
    function __construct() {
        ...
        $dir = $this->root_dir . $this->modules_path . $this->brand_name . '/' . $this->family_line . '/VVXLocalization/';
        foreach (glob($dir . "*", GLOB_ONLYDIR) as $directory) {
            foreach (glob($directory . "/*.xml") as $filename) {
                $file = str_replace($dir, '', $filename);
                $this->copy_files[] = 'VVXLocalization/' . $file;
            }
        }
    }
    ...
    function prepare_for_generateconfig() {
        ...
        $this->protected_files = array('overrides/' . $this->mac . '-phone.cfg', 'logs/' . $this->mac . '-boot.log', 'logs/' . $this->mac . '-app.log', 'VVXLocalization', 'overrides/' . $this->mac . '-phone.cfg');
    }
